...
export async function loadComponent(fileName, containerId, maxRetries = 3) {
  ...
  let hasFreshLoaded = false;

  // 1️⃣ Check cache first and inject ONLY if container is empty
  let cachedHtml = null;
  try {
    const cached = JSON.parse(localStorage.getItem(CACHE_KEY));
    if (cached && (Date.now() - cached.timestamp < CACHE_EXPIRY)) {
      cachedHtml = cached.html;
      // Only inject if container is empty to avoid flickering
      if (container.innerHTML.trim().length === 0) {
        container.innerHTML = cachedHtml;
        console.log(`💾 Hydrated ${fileName} from cache`);
      }
    }
  } catch (err) {
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    for (const url of candidatePaths) {
      try {
        ...
        // 🚫 CRITICAL FIX: Only update DOM if content is different
        const currentHtml = container.innerHTML.trim();
        const newHtml = html.trim();
        
        if (currentHtml !== newHtml) {
          container.innerHTML = html;
          console.log(`✅ Loaded fresh ${fileName} from ${url}`);
        } else {
          console.log(`⚡ Fresh ${fileName} matches current content, no DOM update`);
        }
