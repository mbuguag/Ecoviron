...
const DOMAINS = {
  LOCAL: 'localhost',
  PREVIEW: 'vercel.app',
  VERCEL: 'ecoviron.vercel.app',
  STAGING: 'ecoviron-git-master-joseph-mbuguas-projects-d22cf105.vercel.app',
  PRODUCTION: 'www.bionix-hse.co.ke' // Future production domain
};
...
const isStaging = 
  window.location.hostname === DOMAINS.STAGING;

const isProduction = 
  window.location.hostname === DOMAINS.PRODUCTION ||
  window.location.hostname === 'bionix-hse.co.ke';

const isVercelProd = 
  window.location.hostname === DOMAINS.VERCEL && !isPreviewEnv;
...
export const ENV_CONFIG = {
  api: {
    ...
    staging: `https://${DOMAINS.STAGING}/api`,
    production: `https://${DOMAINS.PRODUCTION}/api`,
    vercel: `https://${DOMAINS.VERCEL}/api`
  },
  static: {
  ...
  components: {
    local: "/components/",
    preview: `https://${window.location.hostname}/components/`,
    staging: `https://${DOMAINS.STAGING}/components/`,
    production: `https://${DOMAINS.PRODUCTION}/components/`,
    vercel: `https://${DOMAINS.VERCEL}/components/`
  }
};
...
export const API_BASE_URL = (() => {
  ...
  if (isStaging) return ENV_CONFIG.api.staging;
  if (isProduction) return ENV_CONFIG.api.production;
  if (isVercelProd) return ENV_CONFIG.api.vercel;
  return ENV_CONFIG.api.production; // Fallback
})();
...
export const STATIC_BASE_URL = (() => {
  ...
  if (isStaging) return ENV_CONFIG.static.staging;
  if (isProduction) return ENV_CONFIG.static.production;
  if (isVercelProd) return ENV_CONFIG.static.vercel;
  return ENV_CONFIG.static.production; // Fallback
})();
...
export const BASE_PATH = (() => {
  ...
  if (isStaging) return ENV_CONFIG.basePath.staging;
  if (isProduction) return ENV_CONFIG.basePath.production;
  if (isVercelProd) return ENV_CONFIG.basePath.vercel;
  return ENV_CONFIG.basePath.production; // Fallback
})();
...
export const COMPONENT_BASE_URL = (() => {
  ...
  if (isStaging) return ENV_CONFIG.components.staging;
  if (isProduction) return ENV_CONFIG.components.production;
  if (isVercelProd) return ENV_CONFIG.components.vercel;
  return ENV_CONFIG.components.production; // Fallback
})();
