...
import { addToGuestWishlist, removeFromGuestWishlist, getGuestWishlist } from './wishlist.js';
...
function renderProductDetail(product) {
  const gallery = [product.imageUrl || "default.jpg", ...(product.galleryImages || [])];

  document.getElementById("product-name").textContent = product.name;
  document.getElementById("product-price").textContent = `KES ${product.price.toLocaleString()}`;
  document.getElementById("product-desc").textContent = product.description;
  document.getElementById("product-category").textContent = product.category?.name || "Uncategorized";
  document.getElementById("product-rating").innerHTML = renderStars(product.rating);
  document.getElementById("main-product-image").src = `${API_BASE_URL}/uploads/${gallery[0]}`;
  document.getElementById("main-product-image").alt = `Image of ${product.name}`;
  ...
  gallery.forEach((img) => {
    ...
    thumb.addEventListener("click", () => {
      document.getElementById("main-product-image").src = `${API_BASE_URL}/uploads/${img}`;
    });
...
function updateWishlistIconState(isActive) {
  ...
  btn.querySelector("i").className = `fa ${isActive ? "fa-heart" : "fa-heart-o"}`;
}
