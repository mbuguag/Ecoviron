// Load reusable header and footer with dynamic relative path
...
// Calculate path depth and resolve component path accordingly
function getComponentPath(filename) {
  const currentPath = window.location.pathname;
  const depth = currentPath.split('/').length - 2; // index.html = 0, /services/page.html = 1
  return `${'../'.repeat(depth)}components/${filename}`;
}

window.addEventListener('DOMContentLoaded', () => {
  // Load header and footer with dynamic path resolution
  loadComponent(getComponentPath('header.html'), 'header-container');
  loadComponent(getComponentPath('footer.html'), 'footer-container');
