...
export async function loadComponent(fileName, containerId, maxRetries = 3) {
  ...
  // 1️⃣ Try cached version first (but only inject if container is empty)
  try {
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    for (const url of candidatePaths) {
      try {
        ...
        // 🚫 Avoid double render if cached version already matches
        if (container.innerHTML.trim() !== html.trim()) {
          container.innerHTML = html;
          console.log(`✅ Loaded fresh ${fileName} from ${url}`);
        } else {
          console.log(`⚡ Fresh ${fileName} matches cache, no DOM update`);
        }
