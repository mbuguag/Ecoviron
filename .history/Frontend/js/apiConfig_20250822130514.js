...
function getAssetPath(relativePath) {
  return `${STATIC_BASE_URL.replace(/\/$/, "")}/${relativePath.replace(/^\//, "")}`;
}

// --- Helper: Component loader ---
async function loadComponent(targetSelector, componentPath, fallbackHTML = "") {
  try {
    ...
    if (fallbackHTML) {
      document.querySelector(targetSelector).innerHTML = fallbackHTML;
    }
  }
}
...
async function loadComponents(components) {
  return Promise.all(
    components.map(c => loadComponent(c.selector, c.path, c.fallback))
  );
}
