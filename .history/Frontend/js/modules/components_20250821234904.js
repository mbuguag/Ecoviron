import { BASE_PATH, getAssetPath, isLocalDev, STATIC_BASE_URL,  } from '../apiConfig.js';
...
async function loadComponent(componentFile, container) {
  const url = resolveComponentPath(componentFile);
  console.log(`Fetching component: ${url}`);

  try {
    const response = await fetch(url);
    if (!response.ok) throw new Error(`Failed to fetch ${componentFile} (${response.status})`);
    const html = await response.text();
    container.innerHTML = html;
    return true;
  } catch (err) {
    console.error(`Error loading ${componentFile}:`, err);
    return false;
  }
}
