...
import { COMPONENTS_BASE } from './apiConfig.js';

export async function loadLayoutComponents() {
  try
  {
    console.log(`Loading components from: ${COMPONENTS_BASE}`);

  
    const [header, footer] = await Promise.all([
      loadComponent(`${COMPONENTS_BASE}header.html`, 'header-container'),
      loadComponent(`${COMPONENTS_BASE}footer.html`, 'footer-container')
    ]);
