...
export async function loadComponent(fileName, containerId, maxRetries = 3) {
  ...
  // Candidate paths (cleaned, no duplicate /components/components/)
  ...
  if (!isLocalDev) {
    container.innerHTML = `
      <div class="component-error" style="
        padding: 1rem; background: rgba(220,53,69,0.1);
        border: 1px solid rgba(220,53,69,0.3); border-radius: 4px;
        color: #721c24; text-align: center; margin: 0.5rem 0;
      ">
        <p>⚠️ ${fileName.replace(".html", "")} failed to load.</p>
