...
// Calculate depth from current location to root
// function getDepth() {
//   const path = window.location.pathname;
//   return path.split('/').filter(Boolean).length - 1; // index.html = 0, /services/page.html = 1
// }

function getComponentPath(filename) {
  const currentPath = window.location.pathname;
  const depth = currentPath.replace(/\/$/, '').split('/').length - 1;
  return `${'../'.repeat(depth)}components/${filename}`;
}


// Generate dynamic path to target
function resolvePath(relativePath) {
  return `${'../'.repeat(getDepth())}${relativePath}`;
}
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (carouselSlide) {
    services.forEach(service => {
      ...
      slide.innerHTML = `
        <img src="${resolvePath(service.image)}" alt="${service.title}">
        <h3>${service.title}</h3>
      `;
