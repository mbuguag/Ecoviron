...
export async function loadComponent(relativePath, containerId, retries = 3) {
  try {
    ...
    // 🔑 Auto-detect: try both `components/` and `frontend/components/`
    const candidatePaths = [
      resolvePath(relativePath),
      resolvePath(`../${relativePath}`), // e.g. /components/header.html
      resolvePath(`frontend/${relativePath}`),
      resolvePath(`../frontend/${relativePath}`),// e.g. /frontend/components/header.html
    ];
    ...
    if (!html) throw new Error(`All candidate paths failed for ${relativePath} (last tried ${urlTried})`);
