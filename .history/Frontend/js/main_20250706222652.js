...
// Load quote modal into pages dynamically
export async function loadQuoteModal() {
  const modalContainer = document.createElement('div');
  try {
    const response = await fetch('../partials/quote-modal.html'); // adjust path as needed
    const html = await response.text();
    modalContainer.innerHTML = html;
    document.body.appendChild(modalContainer);

    // Modal toggle functionality
    document.getElementById('openQuoteModal')?.addEventListener('click', e => {
      e.preventDefault();
