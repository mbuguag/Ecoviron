...
function detectComponentsBasePath() {
  ...
  if (isLocalDev) {
    // Check if we're in the js/modules directory and adjust path accordingly
    const currentPath = window.location.pathname;
    if (currentPath.includes('/js/') || currentPath.includes('/modules/')) {
      return '../../components/';
    }
    return '../components/';
  }
...
export async function loadComponent(fileName, containerId, maxRetries = 3) {
  ...
  const candidatePaths = [
    ...
    // Then try common relative paths
    `../components/${fileName}`,
    `./components/${fileName}`,
    `../../components/${fileName}`,
  ...
  while (retryCount < maxRetries) {
    for (const url of candidatePaths) {
      try {
        console.log(`Trying to load component from: ${url}`);
