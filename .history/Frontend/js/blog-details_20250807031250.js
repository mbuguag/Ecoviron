...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    if (!sessionStorage.getItem(viewedKey)) {
      try {
        await fetch(`http://localhost:8080/api/public-blogs/public/${postId}/views`, {
          method: "PUT",
        });

        //  Re-fetch the post to get updated view count
        const updatedPostResponse = await fetch(
          `http://localhost:8080/api/public-blogs/public/${postId}`
        );
        post = await updatedPostResponse.json();
  ...
  function renderPost(post) {
    ...
    document.getElementById("post-meta").innerHTML = `
      <div class="author-info">
        <img src="${
          post.author?.profilePicture || "../assets/images/blog.jpg"
        }" 
    ...
    document.getElementById("post-content").innerHTML = `
      <div class="featured-image">
        <img src="${post.imageUrl || "../assets/images/blog.jpg"}" 
  ...
  function setMetaTags(post) {
    ...
    if (metaDescription)
      metaDescription.content = post.metaDescription || post.snippet || post.title;
    if (ogTitle) ogTitle.content = post.title;
    if (ogDescription) ogDescription.content = post.metaDescription || post.snippet || post.title;
    if (ogImage) ogImage.content = post.imageUrl || "https://www.bionix-hse.co.ke/images/og-default.jpg";
    if (twitterTitle) twitterTitle.content = post.title;
    if (twitterDescription) twitterDescription.content = post.metaDescription || post.snippet || post.title;
    if (twitterImage) twitterImage.content = post.imageUrl || "https://www.bionix-hse.co.ke/images/twitter-default.jpg";
    ...
    if (ldJson) {
      const structuredData = {
        ...
        image: post.imageUrl || "https://www.bionix-hse.co.ke/images/schema-default.jpg",
      };
