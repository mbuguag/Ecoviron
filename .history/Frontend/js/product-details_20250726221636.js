...
function renderProductDetail(product) {
 ...
 mainImage.alt = product.name;
  ...
  //   const categoryLink = `<a href="products.html?category=${product.category.id}">${product.category.name}</a>`;
  ...
  async function renderCategoryBreadcrumb() {
    ...
    if (!categoryId) {
      breadcrumb.innerHTML = `<a href="index.html">Home</a> / <span class="current">All Products</span>`;
      return;
    }
    ...
    try {
      ...
      const homeLink = `<a href="index.html">Home</a>`;
      const current = `<span class="current">${category.name}</span>`;

      breadcrumb.innerHTML = `${homeLink} / ${current}`;
    } catch (err) {
      console.warn("Breadcrumb error:", err);
      breadcrumb.innerHTML = `<a href="index.html">Home</a> / <span class="current">Category</span>`;
    }
  }
