...
// If deployed under subfolder → infer base path automatically
let BASE_PATH = "/";
if (!isLocalDev) {
  // If your site might be deployed in a subdirectory like /frontend/
  const match = currentPath.match(/^\/(frontend|staging|preview)\//);
  BASE_PATH = match ? `/${match[1]}/` : "/";
}

// Final configuration
const API_BASE_URL = isLocalDev ? "http://localhost:8080/api" : `${BASE_PATH}api`;
...
export const config = {
  ...
  API_BASE_URL,
  STATIC_BASE_URL,
  COMPONENTS_BASE: `${BASE_PATH}components/`
};
