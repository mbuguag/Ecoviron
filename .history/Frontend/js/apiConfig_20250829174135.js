...
export async function loadComponent(fileName, containerId, maxRetries = 3) {
  ...
  // 1️⃣ Hydrate from cache if available
  let cachedHtml = null;
  try {
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    for (const url of candidatePaths) {
      try {
        ...
        // 🚫 Only update DOM if different (prevents flicker)
        ...
        if (currentHtml !== newHtml) {
          ...
          console.log(
            `⚡ Fresh ${fileName} matches current content, skipped re-render`
          );
        }
 ...
 * (batch summary logs only once at the end)
 */
