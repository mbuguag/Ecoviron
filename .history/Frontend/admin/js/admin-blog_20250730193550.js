...
async function loadPosts() {
  try {
    const response = await authFetch(API_BASE.blogs); // GET /api/admin-blogs

    if (!response.ok) {
      throw new Error("Failed to load blog posts");
    }

    const posts = await response.json(); // ✅ properly parse the response
    console.log("Loaded posts:", posts);
    renderPosts(posts); // whatever rendering logic you have
  } catch (err) {
