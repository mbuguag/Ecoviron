...
async function handleSubmit(e) {
  ...
  const content = quill.root.innerHTML.trim(); // Get HTML content

  if (!title || !snippet || !content || !imageFile) {
    showStatus("All fields (except link) are required.", true);
    return;
  }
  ...
  try {
    ...
    const imageRes = await fetch(`${API_BASE_URL}/images/blog`, {
      method: "POST",
      body: formData,
    });
    ...
    // 2. Create blog post
    const blogPayload = { title, snippet, link, imageUrl, content };
    ...
    if (!res.ok) throw new Error("Blog post failed to save");
