  ...
  authFetch(API_BASE.quotes)
    ...
    .then((quotes) => {
      container.innerHTML = quotes
        .map(
          (q) => `
        <div class="quote-card" style="border: 1px solid #ccc; padding: 1rem; border-radius: 8px; margin-bottom: 1rem;">
    ...
    .catch((err) => {
      ...
      container.innerHTML = `<p class="error">Failed to load quote requests.</p>`;
    });
}
