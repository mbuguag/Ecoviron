...
function saveProduct() {
  ...
  if (imageInput.files.length > 0) {
    formData.append("image", imageInput.files[0]); // This must match backend param
  }

  const method = id ? "PUT" : "POST";
  const url = id ? `${API_BASE.products}/${id}` : `${API_BASE.products}/upload`; // Assuming you're using /upload endpoint
