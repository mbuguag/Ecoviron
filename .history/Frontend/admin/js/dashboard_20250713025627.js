...
export function authFetch(url, options = {}) {
  ...
  return fetch(url, {
    ...
    if (!res.ok) {
      // Optionally log or alert the error status
    ...
    return Promise.resolve(); // For empty/no-content responses
  });
}
...
function loadProducts() {
  authFetch(API_BASE.products).then((products) => {
    const tbody = document.querySelector("#productTable tbody");
    tbody.innerHTML = "";

    products.forEach(async (p) => {
      // Optional: fetch order count for each product
