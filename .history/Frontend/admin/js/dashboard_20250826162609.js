// Import configuration from apiconfig.js
import { 
  API_BASE_URL, 
  STATIC_BASE_URL, 
  BASE_PATH,
...
// API endpoints based on imported configuration
      ...
      products.forEach(async (p) => {
        try {
          ...
          row.innerHTML = `
            ...
            <td><img src="${getAssetPath(p.imageUrl)}" width="50" /></td>
            <td>${orderCount}</td>
            <td>
    ...
    .then((blogs) => {
      ...
      blogs.forEach((b) => {
        ...
        row.innerHTML = `
          <td>${b.title}</td>
          <td>${b.snippet}</td>
          <td><img src="${getAssetPath(b.imageUrl)}" width="50"/></td>
          <td>
