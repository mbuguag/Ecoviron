...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadPosts() {
    try {
      ...
      const url = `${API_BASE_URL.blogs}?page=${currentPage - 1}&size=${pageSize}${
  ...
  async function togglePostStatus(postId, currentStatus) {
    ...
    try {
      const response = await authFetch(
        `${API_BASE_URL.blogs}/${postId}/status?status=${newStatus}`,
        {
          method: "PUT",
        }
      );
  ...
  function openEditor(postId = null) {
    ...
    if (isEdit) {
      ...
      if (post) {
        ...
        if (post.imageUrl) {
          const fullImageUrl = `${API_BASE_URL.BACKEND_URL}${post.imageUrl}`;
...
async function savePost(event) {
  ...
  try {
    ...
    const url = isEdit ? `${API_BASE_URL.blogs}/${postId}` : API_BASE_URL.blogs;
  ...
  async function confirmDelete(postId) {
    if (confirm("Are you sure you want to delete this post?")) {
      try {
        const response = await authFetch(`${API_BASE_URL.blogs}/${postId}`, {
          method: "DELETE",
        });
