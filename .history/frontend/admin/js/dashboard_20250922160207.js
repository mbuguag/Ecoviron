// dashboard.js
import { API_ENDPOINTS, API_BASE_URL, authFetch } from "../js/apiConfig.js";
  ...
  authFetch(API_ENDPOINTS.products)
    ...
    .then((products) => {
      ...
      products.forEach(async (p) => {
        try {
          const orderCount = await authFetch(`${API_ENDPOINTS.products}/${p.id}/order-count`).then(safeJson);
...
function saveProduct() {
  ...
  const url = isEditing
    ? `${API_ENDPOINTS.products}/${form.productId.value}`
    : `${API_ENDPOINTS.admin.dashboard}/products/upload`;
...
function deleteProduct(id) {
  authFetch(`${API_ENDPOINTS.products}/${id}/order-count`)
    .then(safeJson)
    .then((count) => {
      if (count > 0) {
      ...
      return authFetch(`${API_ENDPOINTS.products}/${id}`, { method: "DELETE" });
    })
...
function saveBlog() {
  ...
  const handleSave = (imageUrl) => {
    ...
    const url = id ? `${API_ENDPOINTS.admin.blogs}/${id}` : API_ENDPOINTS.admin.blogs;
  ...
  if (imageFile) {
    ...
    authFetch(`${API_BASE_URL}/images/upload/blog`, { method: "POST", body: formData })
      .then((res) => res.text())
      .then(handleSave)
...
function loadBlogs() {
  authFetch(API_ENDPOINTS.admin.blogs)
    .then(safeJson)
    .then((blogs) => {
...
function editBlog(id) {
  authFetch(`${API_ENDPOINTS.admin.blogs}/${id}`)
    .then(safeJson)
    .then((b) => {
...
function deleteBlog(id) {
  ...
  authFetch(`${API_ENDPOINTS.admin.blogs}/${id}`, { method: "DELETE" })
    .then(() => loadBlogs())
    ...
    .then((orders) => {
      ...
      orders.forEach((o) => {
        ...
        row.innerHTML = `
          <td>${o.id}</td>
          <td>${o.userEmail}</td>
          <td>KES ${parseFloat(o.totalAmount || 0).toFixed(2)}</td>
          <td>${o.status}</td>
...
function markShipped(id) {
  authFetch(`${API_ENDPOINTS.orders}/${id}/status`, {
    method: "PUT",
...
// === Users ===
function loadUsers() {
  authFetch(API_ENDPOINTS.admin.users)
    .then(safeJson)
    .then((users) => {
...
function loadContactMessages() {
  authFetch(API_ENDPOINTS.admin.contactMessages)
    .then(safeJson)
    .then((messages) => {
...
// === Quotes ===
function loadQuotes() {
  authFetch(API_ENDPOINTS.admin.quotes)
    .then(safeJson)
    .then((quotes) => {
