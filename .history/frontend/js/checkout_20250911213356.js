...
import { isLoggedIn } from "./auth-ui.js";
import { CartAPI } from "./cart/cart-api.js";

// ✅ Import environment config
import { API_BASE_URL, STATIC_BASE_URL, formatPrice } from "./apiConfig.js";

// ✅ Environment-aware success URL
const ORDER_SUCCESS_URL = `${STATIC_BASE_URL}/order-success.html`;
...
async function renderCheckoutSummary(summaryContainer, form) {
  ...
  for (const item of items) {
    html += `
      <li class="checkout-item">
        ...
        <span class="item-quantity">${item.quantity} x ${formatPrice(item.price, "KES")}</span>
        <span class="item-total">${formatPrice(item.itemTotal, "KES")}</span>
      </li>`;
  }
  ...
  html += `</ul>
    <div class="order-total">
      <span>Total:</span>
      <strong>${formatPrice(total, "KES")}</strong>
    </div>`;
...
async function submitOrder(event, refs) {
  ...
  try {
    ...
    if (invalidItems.length > 0) {
      ...
      throw new Error(`The following items are no longer available: ${missingNames}`);
    }
